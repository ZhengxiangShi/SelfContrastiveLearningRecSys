...
class ItemConv(Module):
    ...
    def forward(self, adjacency, embedding):
        ...
        # final = [item_embedding_layer0]  # : final.shape = torch.Size([40727, 100])
        final = item_embedding_layer0
        for i in range(self.layers):
            item_embeddings = trans_to_cuda(self.w_item['weight_item%d' % (i)])(item_embeddings)
            item_embeddings = torch.sparse.mm(trans_to_cuda(adjacency), item_embeddings)
            # final.append(F.normalize(item_embeddings, dim=-1, p=2))  # : item_embeddings.shape = torch.Size([40727, 100])
            final = torch.add(F.normalize(item_embeddings, dim=-1, p=2), final) 
...
class SessConv(Module):
    ...
    def forward(self, item_embedding, D, A, session_item, session_len):
        ...
        # DA = torch.mm(D, A.T).float()  # : change 
        DA = torch.mm(D, A).float()
        for i in range(self.layers):
...
class COTREC(Module):
    def __init__(self, adjacency, n_node, lr, layers, l2, beta,
                 lam, eps, dataset, emb_size=100, batch_size=100,
                 temperature=0.1, item_cl_loss_weight=0.1,
                 sess_cl_loss_weight=0.1, use_item_cl_loss=False,
                 use_sess_cl_loss=False, sampled_item_size=1000,
                 random_search=False, top_k=10, remove_original_cl_loss=False):
        ...
        self.sess_cl_loss_weight = sess_cl_loss_weight
        self.use_item_cl_loss = use_item_cl_loss
        self.use_sess_cl_loss = use_sess_cl_loss
        self.sampled_item_size = sampled_item_size
        self.random_search = random_search
        self.remove_original_cl_loss = remove_original_cl_loss
        self.top_k = top_k
    ...
    def generate_sess_emb_npos(self, item_embedding, session_item, session_len, reversed_sess_item, mask):
        ...
        item_embedding = torch.cat([zeros, item_embedding], 0)
        get = lambda i: item_embedding[reversed_sess_item[i]]
        # seq_h = torch.cuda.FloatTensor(self.batch_size // torch.cuda.device_count(), list(reversed_sess_item.shape)[1], self.emb_size).fill_(0)
        seq_h = torch.cuda.FloatTensor(self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size).fill_(0)
        # seq_h = torch.zeros(self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size)
        for i in torch.arange(session_item.shape[0]):
            seq_h[i] = get(i)
    ...
    def forward(self, session_item, session_len, D, A, reversed_sess_item, mask, epoch, tar, train, diff_mask):
        if train:
            ...
            if self.use_item_cl_loss:
                item_embeddings_i = F.normalize(item_embeddings_i, dim=1)
            ...
            # if self.use_item_cl_loss:
            #     sess_emb_i = F.normalize(sess_emb_i, dim=-1, p=2)
            # else:
                
            sess_emb_i = self.w_k * F.normalize(sess_emb_i, dim=-1, p=2)

            item_embeddings_i = F.normalize(item_embeddings_i, dim=-1, p=2)
            scores_item = torch.mm(sess_emb_i, torch.transpose(item_embeddings_i, 1, 0))  # [bsz, item_size]
            ...
            pos_prob_I = self.example_predicting(item_embeddings_i, sess_emb_i.T)  # : [item_size, 100], sess_emb_i.shape=[bsz, 100]
            pos_prob_S = self.example_predicting(self.embedding.weight, sess_emb_s.T)  # : [item_size, 100]
            ...
            if not self.remove_original_cl_loss:
        ...
        # : compute contrastive loss among session embeddings
        cl_loss = 0
        if self.use_sess_cl_loss:
            bsz, _ = sess_emb_i.shape
            sess_emb_i = F.normalize(sess_emb_i, dim=1)
            sess_logits = torch.div(
                torch.matmul(sess_emb_i, sess_emb_i.T),
                self.temperature)
            if torch.cuda.is_available():
                cl_sess_loss = self.sess_cl_loss_weight * self.loss_function(sess_logits, torch.tensor(range(bsz)).cuda())
            else:
                cl_sess_loss = self.sess_cl_loss_weight * self.loss_function(sess_logits, torch.tensor(range(bsz)))
            cl_loss += cl_sess_loss

        # : compute contrastive loss among item embeddings
        if self.use_item_cl_loss and self.random_search:
            start = random.randint(0, len(item_embeddings_i) - self.sampled_item_size - 1)
            # bs, _ = item_embeddings_i.shape
            sampled_item_embeddings_i = item_embeddings_i[range(start, start + self.sampled_item_size), :]
            logits = torch.div(
                torch.matmul(sampled_item_embeddings_i, sampled_item_embeddings_i.T),
                self.temperature)
            if torch.cuda.is_available():
                cl_item_loss = self.item_cl_loss_weight * self.loss_function(logits, torch.tensor(range(self.sampled_item_size)).cuda())
            else:
                cl_item_loss = self.item_cl_loss_weight * self.loss_function(logits, torch.tensor(range(self.sampled_item_size)))
            cl_loss += cl_item_loss

        if self.use_item_cl_loss and not self.random_search:
            bs, _ = item_embeddings_i.shape
            logits = torch.div(
                torch.matmul(item_embeddings_i, item_embeddings_i.T),
                self.temperature)
            topk_logits, topk_index = torch.topk(logits, k=self.top_k, dim=-1)
            if torch.cuda.is_available():
        ...
        if self.use_item_cl_loss or self.use_sess_cl_loss:
            return self.beta * con_loss, loss_item, scores_item, loss_diff * self.lam, cl_loss
        else:
            return self.beta * con_loss, loss_item, scores_item, loss_diff * self.lam


def forward(model, i, data, epoch, train, use_item_cl_loss=False):
    ...
    if use_item_cl_loss:
        con_loss, loss_item, scores_item, loss_diff, cl_item_loss = model(session_item, session_len, D_hat, A_hat, reversed_sess_item, mask, epoch, tar, train, diff_mask)
        return tar, scores_item, con_loss, loss_item, loss_diff, cl_item_loss
    else:
...
def train_test(model, train_data, test_data, epoch, batch_size=100, use_item_cl_loss=False, use_sess_cl_loss=False):
    print('start training: ', datetime.datetime.now())
    total_loss = 0.0
    slices = train_data.generate_batch(batch_size)
    for i in tqdm.tqdm(slices):
        model.zero_grad()
        if use_item_cl_loss or use_sess_cl_loss:
            tar, scores_item, con_loss, loss_item, loss_diff, cl_item_loss = forward(model, i, train_data, epoch, train=True, use_item_cl_loss=True)
            loss = loss_item + con_loss + loss_diff + cl_item_loss
        else:
    ...
    for i in slices:
        if use_item_cl_loss or use_sess_cl_loss:
            tar, scores_item, con_loss, loss_item, loss_diff, cl_item_loss = forward(model, i, test_data, epoch, train=False, use_item_cl_loss=True)
        else:
